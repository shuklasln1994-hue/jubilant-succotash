...
export default function OrderForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Submitting order to Shiprocket API...')
      ...
      const response = await fetch('/api/shiprocket', {
        ...
        body: JSON.stringify({ 
          ...formData, 
          price, 
          dimensions: formData.dimensions || '10x10x10' 
        }),
      });

      const result = await response.json();
      console.log('Shiprocket API Response:', result);

      if (!response.ok) {
        throw new Error(result.message || 'Failed to create Shiprocket order');
      }

      // ISSUE 2 FIX: Properly extract AWB and courier info from API response
      const awbCode = result.awbCode || result.shipmentId || 'PENDING';
      const courierName = result.courierName || 'NEXYE Courier';
      ...
      // Prepare comprehensive data for confirmation page
      const orderData = {
        // Basic order info
        price: price,
        ...
        // CRITICAL: AWB and Courier info from Shiprocket API
        awbCode: awbCode,
        courierName: courierName,
        
        // Additional Shiprocket info
        shiprocketOrderId: orderId,
        shiprocketShipmentId: shipmentId,
        pickupLocation: result.pickupLocation || `${formData.senderAddress}, ${formData.senderPincode}`,
        
        // Order processing info
        orderCreated: new Date().toISOString(),
        estimatedDelivery: formData.serviceType === 'express' ? '1-2 Business Days' : '2-4 Business Days'
      };

      console.log('Order data being passed to confirmation page:', orderData);
      
      // Navigate to confirmation page with comprehensive order data
